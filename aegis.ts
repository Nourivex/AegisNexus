...
const ALLOWED_MODELS = ["gpt-5-mini", "gpt-4o", "claude-3.7-sonnet", "claude-3.5-sonnet"];
...
function parseExpiresAtMs(value: unknown): number {
...
function deriveBaseUrlFromSessionToken(sessionToken: string): string {
...
async function pollAccessToken(params: {
  ...
  const expiresAt = Date.now() + params.expiresInSec * 1000;
  let delayMs = Math.max(1000, params.intervalSec * 1000);
...
async function exchangeCopilotSession(githubAccessToken: string): Promise<{
  token: string;
  baseUrl: string;
  expiresAt: number;
}> {
...
async function loadToken(paths: ReturnType<typeof resolveWorkspacePaths>): Promise<TokenStore> {
  const raw = await fsp.readFile(paths.tokenFile, "utf8");
...
async function saveToken(paths: ReturnType<typeof resolveWorkspacePaths>, token: TokenStore): Promise<void> {
  await fsp.mkdir(path.dirname(paths.tokenFile), { recursive: true });
  await fsp.writeFile(paths.tokenFile, `${JSON.stringify(token, null, 2)}\n`, "utf8");
}

async function ensureToken(paths: ReturnType<typeof resolveWorkspacePaths>): Promise<TokenStore> {
  try {
...
async function fetchModels(token: TokenStore): Promise<string[]> {
  ...
  return rows
    .map((row) => (row && typeof row === "object" ? String((row as JsonRecord).id || "").trim() : ""))
    .filter(Boolean);
}
...
async function configureSkills(): Promise<void> {
...
async function healthCheck(): Promise<void> {
  ...
  if (fs.existsSync(ws.paths.pidFile)) {
    try {
      const pidRaw = await fsp.readFile(ws.paths.pidFile, "utf8");
...
async function runForegroundCommand(command: string, args: string[], cwd: string): Promise<void> {
  const effectiveCommand = process.platform === "win32" && command === "npm" ? "npm.cmd" : command;

  await new Promise<void>((resolve, reject) => {
    const child =
      process.platform === "win32"
        ? spawn(process.env.ComSpec || "cmd.exe", ["/d", "/s", "/c", [effectiveCommand, ...args].join(" ")], {
            cwd,
            stdio: "inherit",
          })
...
async function gatewayStart(): Promise<void> {
  ...
  const compiledServer = path.join(ws.paths.projectRoot, "dist", "server.js");
  ...
  await fsp.writeFile(ws.paths.pidFile, `${child.pid}\n`, "utf8");
  success(`Gateway berjalan di background (PID ${child.pid}).`);
}

async function gatewayStop(): Promise<void> {
  header("Gateway Stop");
  const ws = await ensureWorkspace();
  ...
  const pid = Number.parseInt((await fsp.readFile(ws.paths.pidFile, "utf8")).trim(), 10);
  if (!isPidRunning(pid)) {
    await fsp.rm(ws.paths.pidFile, { force: true });
...
async function gatewayRestart(): Promise<void> {
  await gatewayStop();
  await gatewayStart();
}
...
function createProgram(): Command {
  ...
  program
    .command("gateway")
    .description("Start/stop/restart AegisNexus server daemon")
