...
import { ensureWorkspace, readWorkspaceConfig, resolveWorkspacePaths } from "./workspace.js";
...
const memoryDb = initDatabase({ baseDir: WORKSPACE_PATHS.memoryDir, dbFileName: "aegisnexus.db" });

async function loadWorkspaceRuntimeConfig(): Promise<{ selectedModel: string; sessionKey: string }> {
...
async function runOrchestration(params: {
  ...
  const selectedModel =
    workspaceCfg.selectedModel ||
    (preferred.includes(persona.defaultModel) ? persona.defaultModel : preferred[0]);
...
void ensureWorkspace(WORKSPACE_PATHS.workspaceRoot)
  .then(() => {
