import type { AgentControl } from "../components/sidebar.js";
...
type DashboardOptions = {
  ...
  getSessionId: () => string;
  onExecutionUpdate: (execution: ChatResult["execution"]) => void;
};
...
function mustQuery<T extends Element>(container: ParentNode, selector: string): T {
...
export function createDashboard(options: DashboardOptions): DashboardController {
  ...
  const chatList = mustQuery<HTMLDivElement>(element, "#chatList");
  const promptInput = mustQuery<HTMLTextAreaElement>(element, "#promptInput");
  const sendBtn = mustQuery<HTMLButtonElement>(element, "#sendBtn");
  const continueBtn = mustQuery<HTMLButtonElement>(element, "#continueBtn");
  ...
  function addChat(role: ChatRole, text: string): void {
